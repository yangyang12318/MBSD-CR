...
class DenoiseDataSet(Dataset):
    def __init__(self, add_noise:str=None, crop_size:list=None, aug:list=None, n_repeat:int=1, n_data:int=None, ratio_data:float=None) -> None:
        ...
        '''
        self.dataset_dir = '/home/peter/yy/dataset'
...
###每增加一个数据，这里都要添加一下，以便随机裁剪相同的尺寸
        # crop
        if 'NLsar' in data:
            data['NLsar'] = data['NLsar'][:, y:y+crop_size[1], x:x+crop_size[0]]
        if 'real_noisy' in data:
            data['real_noisy'] = data['real_noisy'][:, y:y+crop_size[1], x:x+crop_size[0]]
        if 'opt' in data:
            data['opt'] = data['opt'][:, y:y+crop_size[1], x:x+crop_size[0]]
        if 'NLopt' in data:
            data['NLopt'] = data['NLopt'][:, y:y+crop_size[1], x:x+crop_size[0]]
