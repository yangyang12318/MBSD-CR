...
class SARdata(DenoiseDataSet):
    ...
    #QXSLAB_SAROPT
    def _scan(self):
        self.dataset_path = os.path.join(self.dataset_dir, 'train')  # 这里先用测试数据，记得修改回来
    ...
    def _load_data(self, data_idx):
        ...
        #opt_img=self._load_img(os.path.join(self.dataset_path, 'opt_256_oc_0.2', file_name))
        NLsar = self._load_img(os.path.join(self.dataset_path, 'NLsar', file_name), as_gray=True)
        #NLopt=self._load_img(os.path.join(self.dataset_path, 'NLopt', file_name), as_gray=True)
