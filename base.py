...
class BaseTrainer():
    ...
    def _set_module(self):  # 这个在新的函数中重载了
        # return dict form with model name.
        raise NotImplementedError('define this function for each trainer')

    def _set_optimizer(self):  # 这个在新的函数中重载了
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.train_cfg = cfg['training']  # dataset: prep_NIND
        self.val_cfg = cfg['validation']  # dataset: prep_NIND
        self.test_cfg = cfg['test']  # dataset: prep_NIND
    ...
    def _warmup(self):
        ...
        warmup_iter = self.train_cfg['warmup_iter']  # warmup_iter: 200
    ...
    def _before_train(self):
        ...
        self.train_dataloader = self._set_dataloader(self.train_cfg, batch_size=self.train_cfg['batch_size'],
                                                     shuffle=True, num_workers=self.cfg['thread'])
        ...
        # other configuration
        self.max_epoch = self.train_cfg['max_epoch']  # 这个是设置训练的epoch
        self.epoch = self.start_epoch = 1
        # 计算一下总共有多少张图片，就需要迭代几次
        max_len = self.train_dataloader[
            'dataset'].dataset.__len__()  # base number of iteration works for dataset named 'dataset'
        #
        self.max_iter = math.ceil(max_len / self.train_cfg['batch_size'])  # 用总共的图片数量除以设置的batch size 然后向上取整
    ...
    def _run_step(self):
        ...
        # to device
        if self.cfg['gpu'] != 'None':
            for dataset_key in data:  # 这里data是个健值对，有三个键，（opt_img, real_noisy，noisy）
        ...
        # data上面有三个键，clean，real_noisy,还有一个noisy，我不知道noisy哪里来的  我现在知道了，denoise_dataset里面的
    ...
    def test_dataloader_process(self, dataloader, add_con=0., floor=False, img_save=True, img_save_path=None,
        ...
        for idx, data in enumerate(dataloader):
            ...
            file_name = int(data['file_name'][0][:-4])
            # forward
            ...
            if img_save:
                ...
                # denoi_name = '%04d_DN_%.2f'%(file_name, psnr_value) if 'clean' in data else '%04d_DN'%file_name
                denoi_name = '%d_DN' % file_name
                # imwrite
                if 'clean' in data:         self.file_manager.save_img_tensor(img_save_path, '%d_CL' % file_name,
                                                                              clean_img)
                if noisy_img is not None: self.file_manager.save_img_tensor(img_save_path, '%d_N' % file_name,
                                                                            noisy_img)
            ...
            if info:
                if 'clean' in data:
                    ...
                    self.status, file_name, dataloader.__len__(), psnr_value), end='\r')
                else:
                    self.logger.note('[%s] testing... %04d/%04d.' % (self.status, file_name, dataloader.__len__()),
                                     end='\r')

        # final log msg
        if count > 0:
            self.logger.val(
                '[%s] Done! PSNR : %.2f dB, SSIM : %.3f' % (self.status, psnr_sum / count, ssim_sum / count))
        else:
